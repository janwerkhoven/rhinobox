...
$(document).ready(function() {
  ...
  $('#prijs .postcode.levering input, #prijs .postcode.ophaling input').on('keyup blur change', function(e) {
    var str = '';
    if ($(this).val().length == 4) {
      var search = parseInt($(this).val(), 0);
      var array = [];
      for (var i in postcodes) {
        if (postcodes[i][0] === search) {
          array.push(postcodes[i][1]);
        }
      }
      str = 'onbekende postcode';
      if (array.length) {
        for (var ii in array) {
          console.log(ii);
          if (ii === '0') {
